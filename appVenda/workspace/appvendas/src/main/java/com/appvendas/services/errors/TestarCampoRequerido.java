package com.appvendas.services.errors;
...
public class TestarCampoRequerido {
	...
	public static List<ErrosData> validarCampoRequerido(final Object objeto) {
		
		List<ErrosData> errorsData = new ArrayList<>();
		...
		for (int i = 0; i < field.length; i++) {
			...
			if (field[i].isAnnotationPresent(CampoRequerido.class)) {
				...
				try {
					...
					if (conteudoInformadoNoCampo(campo)) {
						
						ErrosData erro= new ErrosData();
